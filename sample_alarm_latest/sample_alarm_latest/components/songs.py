...
def get_random_songs():
    for path, dirs, files in os.walk(os.path.dirname(os.path.dirname(os.path.abspath(__file__))) + default_properties['songs_path']):
        for f in files:
            songs_collection.append(f)

    random_selected_songs = random.choice(songs_collection)

    return os.path.dirname(os.path.dirname(os.path.abspath(__file__))) + default_properties['songs_path'] + random_selected_songs
